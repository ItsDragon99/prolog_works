...
# Practica para el reto de usar alternativas de condiciones en diferentes problemas que propuso el profesor

## Introduccion con explicación de `when`, `case`, `unless` y `cond` en Lisp  
### `when`  
Se ejecuta cuando el código solo si la condición es verdadera. 
### `unless`
Hace lo contrario de when, ejecutando el código solo si la condición es falsa.
### `cond`
Evalúa varias condiciones en orden y ejecuta la primera que se cumpla. Vi que es útil cuando hay múltiples casos posibles.
### `case`
Compara un valor con distintas opciones y ejecuta el código del caso que coincida. Es similar a un switch en otros lenguajes.
